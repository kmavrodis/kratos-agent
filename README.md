...
| **Finance Close Controller** | `use-cases/finance-close/` | Controller co-pilot for month-end close — variance review vs prior year, JE-queue review (surfaces manual + draft entries for scrutiny), cost-centre brief, vendor lookup, and proposing accrual / reclass JEs with explicit user confirmation before posting to the ledger — against the in-repo `sap-s4-mcp-server` mock |
